        ...
        def artist_list_create(list_item):
            ...
            while True:
                ...
                if item_character == "-":
                    ...
                    feature_check = check_features(list_item[0:character_number_artist])
...
def add_to_playlist(playlist_name=None,  music_list=[],playlist_id = None):
    ...
    if playlist_name != None:
        playlist_search_result = sp.search(playlist_name , limit=20, type='playlist')['playlists']['items']
    ...
    for track_nr in range(len(music_list[1])):
        ...
        track_search_querry = music_list[0][track_nr] + " " +music_list[1][track_nr]
        #search with the searchquerry
        track_search_result = sp.search(track_search_querry, limit=1, type='track')
        #append the song id to the id-list
        if track_search_result['tracks']['items'] != []:
            music_list_ids.append(track_search_result['tracks']['items'][0]['id'])
        else:
            print(track_search_querry+" konnte leider nicht gefunden werden... Füge es manuell hinzu!")
...
def create_and_add_playlist(playlist_name, music_list):
    ...
    playlist_id = (sp.user_playlist_create(username , name=playlist_name, public=True))['id']
    ...
    while True:
        ...
        if choice == "2":
            ...
            try:
                ...
                add_to_playlist(playlist_name=name,music_list=m3u_to_list(path))
